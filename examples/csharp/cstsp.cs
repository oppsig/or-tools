...
class Tsp {
	internal class MainSearchMonitor : SearchMonitor {
		public MainSearchMonitor(Solver s) : base(s) { }

		public override bool AcceptDelta(Assignment delta, Assignment deltadelta)	{
		  return base.AcceptDelta(delta, deltadelta);
		}
	}
  ...
  static void Solve(int size, int forbidden, int seed) {
    RoutingModel routing = new RoutingModel(size, 1, 0);
		MainSearchMonitor monitor = new MainSearchMonitor(routing.solver());
  ...
  public static void Main(String[] args) {
    ...
    if (args.Length > 2) {
      seed = Convert.ToInt32(args[2]);
		...
		Console.WriteLine("ToTal Mem:             {0:N0}", GC.GetTotalMemory(true));
		Solve(size, forbidden, seed);
    Console.WriteLine("ToTal Mem (1 Iter):    {0:N0}", GC.GetTotalMemory(false));
		GC.Collect();
    Console.WriteLine("ToTal Mem (GC):        {0:N0}", GC.GetTotalMemory(true));
		...
		while (true) {
			...
			Console.WriteLine("ToTal Mem ({0} Iter): {1:N0}", N, GC.GetTotalMemory(false));
			//GC.Collect();
			//Console.WriteLine("ToTal Mem (GC):       {0:N0}", GC.GetTotalMemory(true));
		}
	}
